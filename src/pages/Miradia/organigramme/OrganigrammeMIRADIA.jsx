// OrganigrammeMIRADIAPro.jsx  — version responsive
...
function AnimatedBackground() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <div className="absolute -top-24 -left-24 h-80 w-80 rounded-full blur-3xl opacity-30 dark:opacity-18 animate-[float1_12s_ease-in-out_infinite]"
        style={{ background: `radial-gradient(circle at 30% 30%, ${MIRADIA.sky}55, transparent 60%)` }} />
      <div className="absolute top-1/3 -right-24 h-96 w-96 rounded-full blur-3xl opacity-24 dark:opacity-15 animate-[float2_14s_ease-in-out_infinite]"
        style={{ background: `radial-gradient(circle at 30% 30%, ${MIRADIA.green}55, transparent 60%)` }} />
      <div className="absolute -bottom-28 left-1/3 h-[520px] w-[520px] rounded-full blur-3xl opacity-18 dark:opacity-12 animate-[float3_16s_ease-in-out_infinite]"
        style={{ background: `radial-gradient(circle at 30% 30%, ${MIRADIA.yellow}44, transparent 60%)` }} />
      <div className="hidden dark:block">
        <div className="absolute top-1/4 -left-12 h-[400px] w-[400px] rounded-full blur-3xl opacity-10 animate-[float4_20s_ease-in-out_infinite]"
          style={{ background: "radial-gradient(circle at 40% 40%, #7C3AED33, transparent 65%)" }} />
        <div className="absolute bottom-1/4 -right-12 h-[450px] w-[450px] rounded-full blur-3xl opacity-08 animate-[float5_22s_ease-in-out_infinite]"
          style={{ background: "radial-gradient(circle at 60% 60%, #4F46E533, transparent 70%)" }} />
      </div>
      <style>{`
...
/* =========================
   HOOK: IntersectionObserver (canvas)
========================= */
...
/* =========================
   CANVAS PersonCard (desktop / tablet)
========================= */
  ...
  return (
    <div className="flex flex-col" style={{ marginLeft: indentPx }}>
      ...
      <div
        ...
        style={{
          border:    `1px solid ${hexToRgba(accentColor, 0.20)}`,
          boxShadow: `0 4px 24px ${hexToRgba(accentColor, isTop ? 0.18 : 0.09)}`,
          ...(isTop ? { background: `linear-gradient(135deg, ${MIRADIA.teal}, ${MIRADIA.navy} 55%, ${MIRADIA.sky})` } : {}),
        }}
      >
        {/* accent top bar */}
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="shrink-0">
            <Avatar person={person} ringColor={accentColor} reveal size="sm" />
          </div>

          {/* Text */}
          <div className="flex-1 min-w-0">
            <div className={cx("text-[13px] font-black leading-tight truncate",
              isTop ? "text-white" : "text-slate-900 dark:text-slate-50")}>
              {full}
            </div>
            <div className={cx("text-[11px] font-semibold truncate",
              isTop ? "text-white/80" : "text-[#124B7C] dark:text-[#3AA6DC]")}>
              {person.role || "—"}
            </div>
            {person.dept && (
              <div className={cx("text-[10px] truncate",
                isTop ? "text-white/65" : "text-slate-500 dark:text-slate-400")}>
                {person.dept}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              style={{
                background: isTop ? "rgba(255,255,255,0.18)" : hexToRgba(accentColor, 0.14),
                color:      isTop ? "#fff" : accentColor,
                border:     `1px solid ${isTop ? "rgba(255,255,255,0.25)" : hexToRgba(accentColor, 0.30)}`,
              }}
            >
              Détails
            </button>
            ...
            {childNodes && (
              <button
                ...
                style={{
                  background: isTop ? "rgba(255,255,255,0.15)" : hexToRgba(accentColor, 0.10),
                  color:      isTop ? "#fff" : accentColor,
                  border:     `1px solid ${isTop ? "rgba(255,255,255,0.20)" : hexToRgba(accentColor, 0.25)}`,
                }}
...
function Modal({ open, person, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <button
        className={cx("absolute inset-0 bg-black/30 dark:bg-black/70 transition-opacity duration-200",
      ...
      <div className="absolute left-1/2 top-1/2 w-[900px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2">
        <div className={cx(
          ...
          "bg-white/90 dark:bg-[#071223]/90 backdrop-blur-2xl",
          "ring-1 ring-black/5 dark:ring-white/10",
          ...
          <div className="h-1.5 w-full"
            style={{ background: `linear-gradient(90deg, ${accentColor}, ${MIRADIA.sky}, ${MIRADIA.green})` }} />

          <div className="p-6 flex items-start justify-between gap-4">
            <div className="flex items-start gap-4 min-w-0">
              <div className="relative shrink-0">
                <div className="h-14 w-14 rounded-2xl overflow-hidden ring-1 ring-black/5 dark:ring-white/10 bg-white/70 dark:bg-white/10"
                  style={{ boxShadow: `0 14px 34px ${hexToRgba(accentColor, 0.18)}` }}>
                  {person.photo
                    ? <img src={person.photo} alt={full} className="h-full w-full object-cover" />
                    : <div className="h-full w-full grid place-items-center font-black text-slate-800 dark:text-slate-50">{initials(full)}</div>
                  }
                </div>
                <div className="absolute -bottom-1 -right-1 h-4 w-4 rounded-full ring-2 ring-white/70 dark:ring-[#071223]/70"
                  style={{ background: accentColor }} />
              </div>
              <div className="min-w-0">
                <div className="text-[20px] font-extrabold text-slate-900 dark:text-slate-50 truncate">{full}</div>
                <div className="mt-0.5 text-[15px] text-slate-700/90 dark:text-slate-200/80 truncate">
                  {person.role}{person.dept ? ` • ${person.dept}` : ""}
                </div>
                {!!person.badge && (
                  <span className="mt-2 inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold text-[#071223] dark:text-slate-50 ring-1 ring-black/5 dark:ring-white/10"
            ...
            <button onClick={onClose}
              className="shrink-0 rounded-2xl px-3 py-2 text-sm font-semibold bg-black/5 hover:bg-black/10 dark:bg-white/10 dark:hover:bg-white/15 ring-1 ring-black/5 dark:ring-white/10 active:scale-[0.98]"
              aria-label="Fermer">✕</button>
          </div>

          <div className="px-6 pb-6 grid gap-4">
            <div className="rounded-2xl p-4 bg-black/3 dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10">
              <div className="text-[13px] font-black tracking-wide text-[#124B7C] dark:text-slate-200">📧 CONTACT</div>
              <div className="mt-3 grid gap-2 text-[13px]">
                <div className="flex items-center justify-between gap-3">
                  <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300"><FiMail className="h-3.5 w-3.5" /> Email</span>
                  {person.email
                    ? <a className="font-semibold hover:underline text-[#025C86] dark:text-[#3AA6DC] truncate max-w-[60%]" href={`mailto:${person.email}`}>{person.email}</a>
                    : <span className="text-slate-400">—</span>
                  }
                </div>
                <div className="flex items-center justify-between gap-3">
                  <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300"><FiPhone className="h-3.5 w-3.5" /> Tél.</span>
                  {person.phone
                    ? <a className="font-semibold hover:underline text-[#025C86] dark:text-[#3AA6DC]" href={`tel:${person.phone.replace(/\s+/g,"")}`}>{person.phone}</a>
                    : <span className="text-slate-400">—</span>
                  }
                </div>
              </div>
            ...
            <div className="rounded-2xl p-4 bg-black/3 dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10">
              <div className="text-[13px] font-black tracking-wide text-[#124B7C] dark:text-slate-200">📝 BIOGRAPHIE</div>
              <div className="mt-3 max-h-[50vh] overflow-auto pr-1 modal-scroll">
                {safeBio
                  ? <div className="rich-bio text-[13px] text-slate-800 dark:text-slate-100" dangerouslySetInnerHTML={{ __html: safeBio }} />
                  : <div className="text-[13px] text-slate-500 dark:text-slate-400">Aucune biographie disponible.</div>
                }
              </div>
            </div>
          </div>
          ...
          <style>{`
            ...
            .modal-scroll::-webkit-scrollbar-thumb{ background:${hexToRgba(accentColor,0.20)}; border-radius:999px; border:2px solid transparent; background-clip:padding-box; }
      ...
      <div className="rounded-3xl overflow-hidden ring-1 ring-black/5 dark:ring-white/10 shadow-[0_30px_90px_rgba(0,0,0,0.14)]">
        <div className="px-6 py-10 text-center text-white"
          ...
          <p className="text-white/85 max-w-2xl mx-auto text-sm md:text-base">
            {isSmall
              ? "Appuyez sur « Détails » pour voir la fiche complète."
          ...
          <div className="mt-6 bg-white/10 p-4 rounded-2xl backdrop-blur-lg max-w-3xl mx-auto flex items-center justify-between flex-wrap gap-3">
            ...
            {/* View toggle — visible on tablet only (JS shows on tablet) */}
            {isSmall && (
              <div className="flex items-center gap-2 rounded-full bg-white/15 p-1 backdrop-blur">
                <button
                  onClick={() => onToggleView("list")}
                  className={cx(
                    "flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold tracking-wide uppercase transition-all",
                    viewMode === "list" ? "bg-white text-[#124B7C] shadow-sm" : "text-white/80 hover:text-white"
                  )}
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className="h-3.5 w-3.5" aria-hidden="true"><line x1="9" y1="6" x2="20" y2="6"/><line x1="9" y1="12" x2="20" y2="12"/><line x1="9" y1="18" x2="20" y2="18"/><circle cx="4" cy="6" r="1.5"/><circle cx="4" cy="12" r="1.5"/><circle cx="4" cy="18" r="1.5"/></svg>
                  Liste
                </button>
                <button
                ...
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" className="h-3.5 w-3.5" aria-hidden="true"><rect x="9" y="2" width="6" height="4" rx="1"/><rect x="2" y="17" width="6" height="4" rx="1"/><rect x="9" y="17" width="6" height="4" rx="1"/><rect x="16" y="17" width="6" height="4" rx="1"/><line x1="12" y1="6" x2="12" y2="11"/><line x1="5" y1="17" x2="5" y2="14"/><line x1="12" y1="17" x2="12" y2="14"/><line x1="19" y1="17" x2="19" y2="14"/><line x1="5" y1="14" x2="19" y2="14"/></svg>
                  Arbre
                </button>
              </div>
            )}
...
function MobileTree({ persons, onOpen }) {
  ...
  // Roots = nodes whose parentId is null OR whose parentId doesn't exist in the known id set
  const knownIds = useMemo(() => new Set(persons.map((p) => p.id)), [persons]);

  const roots = useMemo(() =>
    persons.filter((p) => p.parentId === null || !knownIds.has(p.parentId)),
  [persons, knownIds]);
...
/* =========================
   CANVAS VIEW (desktop + tablet in canvas mode)
========================= */
function CanvasView({ layout, onOpen }) {
  ...
  const { visibleIds, setObservedRef } = useVisibleIds({
    rootRef: scrollRef, ids, threshold: 0.22, rootMargin: "120px",
  });
  ...
  const panelH = useMemo(() =>
    Math.min(vh * 0.88, Math.max(1200, (layout?.height || 0) + 80)), [layout, vh]);
  ...
  return (
    <div className="w-full rounded-3xl bg-white/50 dark:bg-[#0B1626]/55 backdrop-blur-xl ring-1 ring-black/5 dark:ring-white/10 p-3 sm:p-4">
      <div
        ref={scrollRef}
        className="org-scroll pt-48 mt-12 pl-8 overflow-auto rounded-2xl flex items-center justify-center"
        style={{ height: panelH }}
        ...
        <div className="relative flex items-center justify-center"
          ...
          <svg className="absolute inset-0" width={layout.width} height={layout.height}
            ...
            {layout.links.map((l, idx) => {
              ...
              return (
                <path key={idx} d={d} fill="none" stroke="url(#miradiaLink)"
          ...
          {/* Nodes */}
          {layout.items.map((it) => (
            <div key={it.id} data-oid={it.id} ref={setObservedRef(it.id)}
              className="absolute" style={{ left: it.cx, top: it.cy, transform: "translate(-50%, -50%)" }}>
          ...
          <style>{`
            .org-link{ transition: stroke-dashoffset 520ms cubic-bezier(.2,.8,.2,1), opacity 180ms ease; will-change: stroke-dashoffset, opacity; }
            .org-link--on{ transition: stroke-dashoffset 560ms cubic-bezier(.2,.8,.2,1), opacity 160ms ease; }
            ...
            .org-scroll::-webkit-scrollbar-thumb:hover{ background:rgba(148,163,184,0.5); }
            .dark .org-scroll::-webkit-scrollbar-thumb{ background:rgba(148,163,184,0.2); }
...
export default function OrganigrammeMIRADIAPro() {
  ...
  const isSmall  = bp !== "desktop"; // tablet + mobile get the toggle

  // On mobile: default "list". On tablet: default "canvas". Desktop: always "canvas".
  const [viewMode, setViewMode] = useState(() =>
    window.innerWidth < 640 ? "list" : "canvas"
  );

  // When breakpoint changes, enforce sensible defaults
  useEffect(() => {
    if (bp === "desktop")       setViewMode("canvas");
    else if (bp === "mobile")   setViewMode((v) => v); // keep user choice on mobile
  }, [bp]);
  ...
  useEffect(() => {
    ...
    axios.get("/orgnodes/slides", { params: { active: 1 } })
      .then((res) => {
        ...
        if (mounted) setNodes(raw);
      })
      .catch((e) => { if (mounted) { setError(e?.response?.data?.message || e?.message || "Erreur"); setNodes([]); } })
  ...
  const layout = useMemo(() => {
    ...
    const norm = items.map((it) => ({
      ...it,
      cx: PAD + (it.x - minX) * CELL_X,
      cy: PAD + (it.y - minY) * CELL_Y,
    }));
  ...
  /* Persons flat list (for mobile tree) */
  const persons = useMemo(() =>
    (Array.isArray(nodes) ? nodes : []).map(toPerson), [nodes]);

  /* Layout with normalized items for CanvasView */
  const canvasLayout = useMemo(() => {
    if (!layout) return null;
    return { ...layout, items: layout.norm };
  }, [layout]);
        ...
        <div className="relative flex-col w-full max-w-[1700px] px-2 sm:px-4 lg:px-8 py-8">
          ...
          {!loading && !error && nodes.length > 0 && (
            <>
              ...
              {/* ── CANVAS (desktop always, tablet if user picks it) ── */}
