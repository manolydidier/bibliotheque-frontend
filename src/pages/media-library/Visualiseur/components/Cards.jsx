...
export function KpiCard({ label, value, suffix, icon, color = "blue" }) {
  ...
  return (
    <div
      className={`relative group rounded-2xl border bg-gradient-to-br ${colorClasses[color]} backdrop-blur-sm 
      p-6 sm:p-7 lg:p-8 transition-all duration-500 hover:shadow-xl hover:-translate-y-2 hover:scale-[1.02]`}
    >
      <div
        className={`absolute top-0 left-6 lg:left-8 h-1 w-16 bg-gradient-to-r ${gradientClasses[color]} rounded-b-full`}
      />
      <div className="flex items-start justify-between mb-4 lg:mb-6">
        <div className="text-xs font-medium text-slate-600 uppercase tracking-wider">
          {t(label)} {/* ✅ traduction dynamique */}
        </div>
        {icon && (
      ...
      <div className="text-2xl lg:text-3xl font-light text-slate-800 tracking-tight">
        {value ?? "—"}
        {suffix && (
          <span className="text-base lg:text-xl text-slate-500 ml-1">{suffix}</span>
        )}
      </div>
    </div>
  );
...
export function EmptyChart({ message = "noData" }) {
  const { t } = useTranslation(); // ✅ i18n
  return (
    <div className="h-64 md:h-72 xl:h-80 2xl:h-[28rem] flex flex-col items-center justify-center text-slate-400">
      ...
      <p className="text-sm text-center max-w-xs leading-relaxed">{t(message)}</p>
    </div>
  );
}
