...
/* ========================= Helpers ========================= */
const SECTION_IDS = ["hero", "contact"]; // on garde seulement celles qui existent sur HomeMiradia
...
const scrollToSection = (id) => {
  ...
  const offset = (window.scrollY || 0) + rect.top - 90; // offset navbar
  ...
  return (
    <button
      ...
      className={`
        ...
        ${isDark ? "bg-slate-900/80 ring-offset-[#075985]" : "bg-sky-100/90 ring-offset-white"}
      `}
...
const NavBarMiradia = () => {
  ...
  const go = (id) => {
    ...
    // ✅ routes dédiées demandées
    if (id === "plateform") {
      navigate("/librairie-home");
      return;
    }
    if (id === "beneficiaires") {
  ...
  /* Scroll effects + active section (seulement sur Home "/") */
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    const onScroll = () => {
      ...
      const DELTA = 12;
      if (y > 120 && diff > DELTA && !isHidden) setIsHidden(true);
      else if ((diff < -DELTA || y <= 0) && isHidden) setIsHidden(false);
      ...
      for (const sid of SECTION_IDS) {
        ...
        if (!el) continue;
        const rect = el.getBoundingClientRect();
        if (rect.top <= window.innerHeight * 0.35) current = sid;
        else break;
      }
  ...
  const navBaseClasses = isDark
    ? `
      ...
      text-slate-50 border-slate-800/80 shadow-none
    `
    : `
      bg-gradient-to-r from-slate-50/15 via-white/25 to-slate-50/45
      text-slate-900 border-slate-200/90 shadow-none
    `;

  const linkBaseClasses = isDark
    ? "text-blue-50/90 hover:text-white"
    : "text-[#0b5a82] hover:text-[#082f49]";
  const activeLinkClasses = isDark ? "bg-white/10 text-white" : "bg-sky-100 text-[#0b5a82]";
  const progressBarColor = isDark ? "bg-[#22c55e]" : "bg-[#0ea5e9]";

  const isBeneficiairesRoute = location.pathname === "/beneficiaires";
  const isDomainesRoute = location.pathname === "/domaines";
  const isPlateformRoute = location.pathname === "/librairie-home";
  ...
  return (
    <>
      <nav
        className={`
          fixed top-0 left-0 w-full z-50 border-b backdrop-blur-xl px-12 md:px-auto
          ${navBaseClasses} ${heightClass}
          flex items-center transition-all duration-300 ease-out ${translateClass}
        `}
        role="navigation"
            ...
            <div className="flex-1 flex justify-center">
              ...
              >
                ...
                <NavItem
                  ...
                  sectionId="hero"
                  active={isHomeRoute && activeSection === "hero"}
                  onClick={go}
                  compact={isCompact}
                  base={linkBaseClasses}
                  activeClass={activeLinkClasses}
                />
            {/* ✅ scroll contact only on home, sinon retourne home+scroll */}
                <NavItem
                  label={t("nav.contact", "Contact")}
                  sectionId="contact"
                  active={isHomeRoute && activeSection === "contact"}
                  onClick={go}
                  compact={isCompact}
                  base={linkBaseClasses}
                  activeClass={activeLinkClasses}
                ...
                />
                 ...
                 {/* ✅ NEW: Plateform -> /librairie-home */}
                <NavItem
                  label={t("nav.plateform", "Plateform")}
                  sectionId="plateform"
                  active={isPlateformRoute}
                  onClick={go}
                  compact={isCompact}
                  base={linkBaseClasses}
                  activeClass={activeLinkClasses}
          ...
          <div className="flex items-center gap-2 md:gap-3 ml-auto">
            ...
            {isMobile && (
              <button
                ...
                `}
                aria-label={
                  mobileOpen ? t("nav.close_menu", "Fermer le menu") : t("nav.open_menu", "Ouvrir le menu")
                }
              >
                <svg
                  className={`w-6 h-6 transition-transform duration-200 ${isDark ? "text-blue-50" : "text-[#0b5a82]"} ${
      ...
      {isMobile && (
        <>
          <div
            className={`
              fixed inset-0 z-[48] backdrop-blur-sm transition-opacity duration-300
          ...
          <div
            className={`
              fixed inset-x-0 top-20 z-[49] pt-4 pb-6 px-4 rounded-b-2xl shadow-2xl
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => setMobileOpen(false)}
                  className={`
                    h-10 w-10 rounded-lg inline-flex items-center justify-center transition-all duration-200
                    focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/80
  ...
  <li role="none">
    <button
      ...
      className={`
        ...
        text-[15px] md:text-[16px] font-semibold whitespace-nowrap rounded-lg
        transition-all duration-200
        ${base} ${active ? activeClass : ""}
      `}
    >
...
const MobileNavItem = ({ label, sectionId, active, onClick, isDark }) => (
  ...
  >
    <span>{label}</span>
    {active && <span className={`w-1.5 h-1.5 rounded-full ${isDark ? "bg-[#0ea5e9]" : "bg-[#22c55e]"}`} />}
  </button>
);
